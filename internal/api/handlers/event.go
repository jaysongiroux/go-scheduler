...
func (h *Handler) UpdateEvent(w http.ResponseWriter, r *http.Request) {
	...
	if existingEvent.IsRecurringInstance {
		...
		case ScopeAll:
			// Update entire series - update master metadata and regenerate ALL instances
			if existingEvent.MasterEventUID == nil {
				web.RespondError(w, http.StatusBadRequest, "Instance has no master event")
				return
			}
			...
			// Update master metadata/recurrence but preserve its start_ts (recurrence origin)
			...
			web.RespondJSON(w, http.StatusOK, master)
			return
		default:
			web.RespondError(
				w,
	...
	if existingEvent.IsMasterEvent() {
		...
		case ScopeAll:
			// Update master metadata/recurrence, preserve start_ts (recurrence origin)
			existingEvent.Metadata = updateData.Metadata
			if updateData.Recurrence != nil {
